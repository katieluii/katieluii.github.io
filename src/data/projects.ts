...
export const projects: Project[] = [
  ...
  },
  {
    ...
    longDescription: "In my Master's project at Prof Susan Duty's lab, I built a targeted drug-repositioning workflow to surface existing, clinically used compounds that could modify Parkinson’s disease biology by boosting mitochondrial quality control through the PINK1/Parkin (PINK1/PARK2) pathway. Starting from large-scale transcriptional perturbation resources (CMap + LINCS accessed via SPIED3), I designed a stepwise filtering strategy (upregulation strength → BBB suitability → chronic-use safety → clinical utility) to move from dozens of in-silico “hits” to a short, defensible set of candidates. To validate signal beyond gene-expression predictions, I took the finalists into wet-lab testing and quantified PINK1 and Parkin protein changes after treatment using western blotting in MCF-7 cells. The pipeline converged on three repurposing candidates (ethotoin, oxybutynin, sirolimus), each significantly increasing Parkin expression, with sirolimus additionally increasing PINK1 -- creating a practical bridge from bioinformatics screening to experimentally supported hypotheses for neuroprotection in PD models.",
