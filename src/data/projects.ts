...
export const projects: Project[] = [
  ...
  },
 {
  ...
  },
  {
    ...
    yearEnd: 2026,
    summary:
      "Tells a biotech or consultancy which companies to prioritise meeting at a partnering conference (Jefferies, BIO, JPM, BioEquity): scores every attending company against your own angle, shows the reasoning per row, and drafts a first-pass note you edit before sending. Runs in the browser with Jefferies London 2026 as the default and BIO 2026 retained as a selectable dataset.",
    longDescription:
      "Partnering conferences put thousands of companies in one place, and the platform's own AI matchmaker is generic. This engine builds the prioritisation matrix from company profiles instead: it takes a client's positioning from the public domain (their website), pulls the conference's attending-company list, and scores each company against what the client is actually looking for. Scoring is two-tier and deterministic — hard MUST filters (partner type, therapeutic-area fit, deal type) gate the universe, then weighted PREFER rules (stage fit, strategic complementarity, BD appetite, financial capacity) produce a 0–100 fit score, so every rank comes with a readable rationale rather than a black-box number. The public demo defaults to the Jefferies London 2026 roster pulled on 21 September 2026: 330 company entries and 899 published attendee records, with missing fields left unknown. The existing BIO 2026 exhibitor directory remains selectable with all 1,654 companies from its 21 June 2026 pull. The output is a portal that runs entirely in the browser: a ranked shortlist with the fit shown by rule and a data-completeness count per row, live re-ranking as you edit your angle or the weights, a search over the whole universe that explains why a company is not listed, and a card per company with the reasoning, published Jefferies attendees where available, plus an editable first-pass note in your own greeting and sign-off to approve, export or copy into your mail app. Nothing is sent by the tool.",
