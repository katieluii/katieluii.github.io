...
import { ExposureLegend, ExposureMark, MULT_SHORT, expShapePath, expInk, Tk } from './ui';
        ...
        >
          ...
          {ticks.map((t) => (
            <g key={t}>
              ...
              <text x={xOf(t)} y={AXIS_Y + 20} textAnchor="middle" fontSize={11} fill="var(--faint)" fontFamily="inherit">{t}×</text>
            </g>
          ))}
          <text x={PAD_L} y={H - 4} fontSize={11} fill="var(--faint)" fontFamily="inherit">cheap</text>
          <text x={W - PAD_R} y={H - 4} fontSize={11} fill="var(--faint)" textAnchor="end" fontFamily="inherit">rich</text>
          ...
          {onLadder.map((c, i) => {
            ...
            return (
              <g
                ...
                tabIndex={0}
                role="button"
                aria-label={`${c.name}, ${c.fpe}× forward P/E, ${c.gr} revenue growth`}
              ...
              >
                {lane > 0 && <line x1={x} y1={AXIS_Y - 9} x2={x} y2={ly + 15} stroke="var(--hair)" />}
                <path d={expShapePath(c.exp, on ? 7 : 5.5)} transform={`translate(${x} ${AXIS_Y})`} fill={expInk(c.exp)} />
                <text
                  x={x}
                  y={ly}
                  textAnchor="middle"
      ...
      <ol className="sm:hidden divide-y divide-[var(--hair)] rounded-xl border border-[var(--hair)] bg-[var(--surface)]">
        {[...onLadder].sort((a, b) => (a.fpe as number) - (b.fpe as number)).map((c) => (
          <li key={c.ticker} className="flex items-center gap-3 px-3 py-2 text-[14px]">
            <ExposureMark exp={c.exp} word={false} />
            <span className="w-16 font-semibold tracking-[0.03em] text-[var(--ink-strong)]">{c.ticker}</span>
